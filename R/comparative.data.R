...
comparative.data <- function(phy, data, names.col, vcv = FALSE, vcv.dim = 2,
    ...
    if (is.na(namesInd)) {
        stop(
            ...
            "' not found in data frame '", data.name, "'"
        )
    }
    ...
    if (!ape::is.rooted(phy)) {
        if (force.root) {
            ...
            stop(
                ...
                "' is not rooted or has a basal polytomy."
            )
        }
    }
    ...
    if (length(to.drop) > 0) {
        matchedPhy <- ape::drop.tip(phy, to.drop)
    } else {
        matchedPhy <- phy
    }
    ...
    if (any(is.na(tip.order))) {
        stop(
            "Problem with sorting data frame: ",
            "mismatch between tip labels and data frame labels"
        )
    }
    ...
    if (is.null(matchedPhy$node.label)) {
        ...
        matchedPhy$node.label <- ifelse(
            matchedPhy$node.label == "", NA, matchedPhy$node.label
        )
        if (any(duplicated(na.omit(matchedPhy$node.label)))) {
            stop("Duplicate node labels present in phylogeny")
        }
        matchedPhy$node.label <- ifelse(
            is.na(matchedPhy$node.label), IntNd, matchedPhy$node.label
        )
    }


    # Compile comparative dataset which COULD HAVE
    # phy components as first level slots rather than nested within $phy
...
print.comparative.data <- function(x, ...) {
    ...
    if (!is.null(attr(x, "growTree"))) {
        cat(
            "Simulated using growTree: object contains ",
            "node data and simulation details\n"
        )
    }
    cat("Phylogeny:", x$phy.name, "\n")
    cat("   ", length(x$phy$tip.label), " tips, ",
        x$phy$Nnode, " internal nodes\n  ",
        sep = ""
    )
...
na.omit.comparative.data <- function(object, scope = NULL, ...) {
    ...
    if (!is.null(scope)) {
        if (!is.null(attr(object, "na.omit.scope"))) {
            stop("Scope of comparative data set already set.")
        }
    ...
    if (length(to.drop) == nrow(object$data)) {
        warning("No complete rows present in data.")
    }

    # guard against ape 'feature' of dropping all tips for empty vectors
    if (length(to.drop) > 0) {
        # lose bits of tree
        object$phy <- ape::drop.tip(object$phy, to.drop)
        # lose rows
    ...
    if (!is.null(attr(object, "growTree"))) {
        cat("Warning: subsetting of node data not implemented.\n")
    }
    return(object)
}
...
"[.comparative.data" <- function(x, i, j) {
    ...
    if (!missing(j)) {
        if (is.null(j)) {
            stop("Null indices not permitted on comparative data objects")
        }
    ...
    if (!missing(i) && hasI) {
        ...
        if (is.character(i)) {
            ...
            if (length(toKeep) != length(i)) {
                warning("Some tip names were not found")
            }
            ...
            toKeep <- rownames[toKeep]
            if (!all(abs(i) %in% seq_along(rownames))) {
                warning("Some row numbers were not found")
            }
        } else if (is.logical(i)) {
            if (length(i) != length(rownames)) {
                stop("Logical index does not match number of tips")
            }
            toKeep <- rownames[i]
        }

        # Work out which to drop
        #  - 'drop.tip' will create a pointless tree if asked to drop
        #     nothing from a tree
        rowToKeep <- match(toKeep, rownames)
        if (length(rowToKeep) < 2) {
            stop("Comparative dataset cannot be reduced to fewer than 2 taxa")
        }
        if (length(rowToKeep) != length(rownames)) {
            toDrop <- setdiff(rownames, toKeep)
            # this assumes that drop.tip preserves the remaining order
            # - testing suggests ok
            if (length(rowToKeep) == length(rownames)) {
                warning("No taxa to drop from comparative dataset")
            } else {
                x$phy <- ape::drop.tip(x$phy, toDrop)
            }
...
caicStyleArgs <- function(phy, data, names.col, warn.dropped = FALSE) {
    ...
    #  - allow for the order of the variables to be reversed:
    #    phylo.d, I'm looking at you

    if (argClass[1] == "data.frame") {
        data <- eval(
            substitute(comparative.data(
                phy = phy, data = data, names.col = XXX,
                warn.dropped = warn.dropped
            ), list(XXX = names.col))
        )
    } else {
        data <- eval(
            substitute(comparative.data(
                phy = data, data = phy, names.col = XXX,
                warn.dropped = warn.dropped
            ), list(XXX = names.col))
        )
    }
