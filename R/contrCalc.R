contrCalc <- function(vals, phy, ref.var, picMethod,
                      crunch.brlen, macro = NULL, trimTips = TRUE) {
    ...
    # DESIGN THOUGHTS - simpler to maintain with a single function for
    #                   each type of contrast
    #                 - create an initialize function to do these shared
    #                   first steps?
    ...
    # get a contrast matrix and a table of nodal values for internal
    # and external nodes
    contrMat <- matrix(NA,
        ncol = ncol(vals), nrow = nIntNd,
        dimnames = list(IntNd, colnames(vals))
    )
    ...
    # guard against phylo.d on missing data and
    # zero branch lengths (Will Pearse)
    ...
    if (picMethod == "phylo.d") {
        if (any(is.na(vals))) {
            stop("Missing data in phylo.d call.")
        }
        if (any(phy$edge.length == 0)) {
            stop("Zero branch lengths in phylo.d call.")
        }
    }
    ...
    for (nd in seq_along(contrGp)) {
        ...
        switch(picMethod,
            "crunch" = {
                # can't do anything with _no_ data, but need to do something
                # with one or more children with information...
                if (any(compChild)) {
                    ...
                    if (sum(compChild) > 2) {
                        ...
                        ## think about this - check out Nick Isaac's comment
                        # about only stalling on negative variance at a node.
                        bl <- bl - crunch.brlen
                        if (any(bl < 0)) {
                            stop(
                                "Crunch contrast calculation at a polytomy ",
                                "gives negative branch lengths."
                            )
                        }
                        ...
                        if (var(rv) < .Machine$double.eps) {
                            ...
                            # find groupings a vector indicating whether each
                            # row is bigger or smaller than the mean of
                            # reference variable or is NA
                            # TODO - think >= or >?
                            group <- (rv > mean(rv, na.rm = TRUE))
                        }

                        invbl <- 1 / bl
                        ProdValBl <- vals * invbl
                        ProdValBl <- rbind(
                            colSums(ProdValBl[!group, , drop = FALSE]),
                            colSums(ProdValBl[group, , drop = FALSE])
                        )
                        ...
                        # weighted means
                        currNV <- colSums(subNV * (1 / subBL)) / sum(1 / subBL)
                        currVar <- sum(subBL)
            ...
            "phylo.d" = {
                ...
                #  - just calculates the difference between the parent
                #    and child nodal values
                #  - also there won't be any missing data so can considerably
                #    slimmed down

                if (sum(compChild) == 2) {
                    # dichotomous node - get the nodal values
                    currNV <- colSums(vals * (1 / bl)) / sum(1 / bl)
                    # subtract the parent from the children and sum the
                    # absolute changes
                ...
                if (sum(compChild) > 2) {
                    # This is the pagel method for polytomies
                    # think about this - check out Nick Isaac's comment about
                    # only stalling on negative variance at a node.
                    ...
                    if (var(rv) < .Machine$double.eps) {
                        ...
                        # is bigger or smaller than the mean of reference
                        # variable or is NA # TODO - think >= or >?
                        group <- (rv > mean(rv, na.rm = TRUE))
                    }
                    ...
                    # get the nodal values using weighted means
                    currNV <- colSums(subNV * (1 / subBL)) / sum(1 / subBL)
                    # subtract the parent from the children and sum the
                    # absolute changes
                    currContr <- colSums(abs(t(t(vals) - currNV)))
            ...
            "brunch" = {
                # further exclude any nodes which have been used to
                # calculate contrasts
                compChild <- compChild & !brunchUsed[children]

                # can't do anything with _no_ data, but need to do
                # something with one or more children with information...
                if (any(compChild)) {
                    # continue calculation with those complete rows
                    compVals <- vals[compChild, , drop = FALSE]
                    bl <- bl[compChild]
                    ...
                    if (sum(compChild) == 1) {
                        # only one complete row... pass through to next
                        # node down as with crunch
                        currNV <- compVals
                        currContr <- NA
                        currBlAdj <- bl
                        currVar <- NA
                    ...
                    if (sum(compChild) > 2) {
                        if (var(rv) == 0) { # weighted average of data
                            ...
                            if (any(bl <= 0)) {
                                stop(
                                    "Brunch contrast calculation at a ",
                                    "polytomy gives negative or zero ",
                                    "branch lengths."
                                )
                            }
                            ...
                            # row is bigger or smaller than the mean of
                            # reference variable or is NA
                            # TODO - Hmm. what to do if there is no variance in
                            #        the reference variable
                            # TODO - From CAIC, this looks like gt not geq?
                            group <- (rv > mean(rv, na.rm = TRUE)) #
                ...
                if (any(compChild)) {
                    ...
                    if (sum(compChild) > 2) {
                        if (var(rv) == 0) { # weighted average of data
                            ...
                            # randomly resolve the polytomy with 0 length
                            # branches calculate contrasts at each node and
                            # downweight in the regression possibly applying a
                            # correction in the degrees of freedom.
                            browser()
                            nTips <- sum(compChild)
                            randRes <- ape::rtree(nTips)
        ...
        # Insert nodal values back into the table to be used in more
        # nested nodes
        ...
        if (macro) {
            ...
            if (nrow(vals) > 2 || any(is.na(rv))) {
                ...
                switch(macroMethod,
                    "PDI" = {
                        macroContr <- (vals[1] / sum(vals)) - 0.5
                    },
                    "RRD" = {
                        macroContr <- log(vals[1] / vals[2])
                    }
                )
            }
        ...
        if (sum(compChild) < 2) {
            # i.e. a node with data for one tip being passed through
            # or with no data
            nodeDepth[parent] <- max(nodeDepth[children])
        } else {
            # a contrast has been calculated and this node has a greater depth
    ...
    RET <- list(
        contrMat = contrMat, nodVal = nodVal, var.contr = contrVar,
        nChild = nChild, nodeDepth = nodeDepth
    )
