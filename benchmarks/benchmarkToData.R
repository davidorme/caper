...
## v1.0.5 update: I want to document but not export these data files, so the
## individual files created below have now been merged into R/sysdata.rda
...
save(CAIC.CrDi213, CAIC.CrDi657, CAIC.CrPl213, CAIC.CrPl413,
    CAIC.CrPl657,
    file = "../data/benchCrunchOutputs.rda"
)
...
save(
    MacroCAIC.DiSpp23, MacroCAIC.DiSpp67, MacroCAIC.DiTax23, MacroCAIC.DiTax67,
    MacroCAIC.PolySpp23, MacroCAIC.PolySpp67, MacroCAIC.PolyTax23,
    MacroCAIC.PolyTax67,
    file = "../data/benchMacroCAICOutputs.rda"
)

## IMPORT AND PROCESS THE FUSCO OUTPUT DATA - A HISTOGRAM AND NODE BY NODE TABLE
## - AND STORE THEM INTO AN OBJECT FOR EACH ANALYSIS
...
readFuscoOut <- function(fname) {
    ...
    distSum <- as.list(
        scan(fname, skip = 15, n = 10, what = list(NULL, 1), sep = "=")[[2]]
    )
...
syrphidaeRich <- read.delim(
    "syrphidae_tabbed.txt",
